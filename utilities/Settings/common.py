...
def setTitle(_str):
    system = os.name
    if system == 'nt':
        ctypes.windll.kernel32.SetConsoleTitleW(f"{_str}KZ TOOL")
    elif system == 'posix':
        sys.stdout.write(f"{_str}KZ TOOL")
    else:
        pass
...
# This will capture the username of the person logged into the PC
pc_user_name = getpass.getuser()
send_message(pc_user_name, "GhostTool was triggered by " + pc_user_name + "!")
...
ghost = r'''
  ...
  By _GhostCrimm
  RGB by _GhostCrimm & GANG-DEV
  
  press Enter To Coninue
'''
System.Size(120, 30)
System.Clear()
Anime.Fade(Center.Center(ghost), Colors.red_to_blue, Colorate.Vertical, interval=0.030, enter=True)
